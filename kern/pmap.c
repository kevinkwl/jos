...
boot_alloc(uint32_t n)
{
	...
	if ((uintptr_t)nextfree > KERNBASE + npages * PGSIZE || nextfree < result)
		panic("out of memory, try to allocate %u bytes from %p", n, result);
	return result;
}
...
page_init(void)
{
	...
	//3)
	for (; page2pa(&pages[i]) < EXTPHYSMEM; i++) {
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}
	//4)
	physaddr_t nextfree_pa = PADDR(boot_alloc(0));
	for (; page2pa(&pages[i]) < nextfree_pa; i++) {
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}
	for (; i < npages; i++) {
...
page_free(struct PageInfo *pp)
{
	...
	if (!pp)
		panic("tried to call page_free on NULL");
	if (pp->pp_ref != 0)
		panic("cannot free a page when pp_ref != 0");
	if (pp->pp_link)
		panic("cannot free a page when pp_link != NULL");
...
{
	// LAB 3: Your code here.
	uintptr_t u_va = (uintptr_t)ROUNDDOWN(va, PGSIZE);
	uintptr_t u_va_end = ROUNDUP(u_va + len, PGSIZE);
	perm |= PTE_P;
	for (; u_va < u_va_end; u_va += PGSIZE) {
		pte_t* pte = pgdir_walk(env->env_pgdir, (const void *) u_va, 0);
		if (u_va >= ULIM || pte == NULL || ((*pte) & perm) != perm ) {
			user_mem_check_addr = (u_va < (uint32_t)va ? (uint32_t)va : u_va);
			return -E_FAULT;
		}
